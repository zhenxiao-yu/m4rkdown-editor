 ...
 * Exits cleanly if @vercel/og is unavailable so the build never hard-fails.
 */
...
try {
  ...
  console.warn('⚠ @vercel/og unavailable — skipping OG image generation (pre-committed fallback used)');
  process.exit(0);
}
...
try {
  ...
  console.warn('⚠ OG image render failed — skipping:', err.message);
  process.exit(0);
}
